(function(window, $, undefined){
    ...
    var App = R.getHelper('rosa_movieplayer');

    var movie = {
        NAME : 'RosaMoviePlayer',
        init : function(){
            this.tmpl = App.loadTemplate('tmpl_movie.php');
            this.$R = {};
            //now playing index
            this.playItem = 0;
            this.playList = [];
        ...
        destroy : function(){
            this.$R(this.playListObj.cssSelector.jPlayer).jPlayer('destroy');
            this.$R.dialog('destroy');
        },

        run : function(){
            var id = R.randID('rosa_musicplayer');
            var $R = R.addHTML(this.tmpl('#tmpl_movieplayer', {
                'id':id
            }));
            $R = $R.ui().rosaDialog({
                title : 'MoviePlayer',
                iconClass : 'rosa_icon_video_x_generic_24',
                width : 425,
                minWidth: 482,
                maxWidth: 482,
                height: 100,
